...
/**
 * @brief Вставляет столбец из нулей после всех столбцов, содержащих максимальный элемент в каждой строке двумерного массива.
 * @param arr Ссылка на указатель на двумерный массив целых чисел..
 * @param rows Количество строк в двумерном массиве.
 * @param cols Количество столбцов в двумерном массиве.
 */
 ...
 * @param arr Указатель на исходный двумерный массив, который нужно скопировать.
 * @param rows Количество строк в исходном массиве.
 * @param cols Количество столбцов в исходном массиве.
 * @return int** Указатель на новый двумерный массив, содержащий копию
 */
int** createCopy(int** arr, int rows, int cols);
...
int main() {
    ...
    int** copyArr = createCopy(arr, rows, cols);
    replaceMaxWithNegative(copyArr, rows, cols);
    cout << "Копия массива после замены максимальных элементов на противоположные:" << endl;
    printArray(copyArr, rows, cols);

    // Вставка столбцов из нулей
    insertZeroColumns(&arr, rows, cols);
    cout << "Оригинальный массив после вставки столбцов из нулей:" << endl;
    printArray(arr, rows, cols);

    // Освобождение памяти
    delArray(arr, rows);
    delArray(copyArr, rows);
...
void insertZeroColumns(int*** arr, int& rows, int& cols) 
{
    ...
    for (size_t r = 0; r < rows; ++r) {
        newArr[r] = new int[newCols];
        int maxIndex = -1; 

        for (size_t c = 0; c < cols; ++c) {
            if (maxIndex == -1 || (*arr)[r][c] > (*arr)[r][maxIndex]) {
                maxIndex = c;
            }
        }
...
// Функция для создания копии двумерного массива
int** createCopy(int** arr, int rows, int cols) {
