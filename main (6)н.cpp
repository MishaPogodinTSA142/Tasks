...
void replaceLastKWithOpposite(int* arr, const int n, const int k) 
{
    ...
    int elementsToReplace = (k > n) ? n : k;

    // Заменяем последние k элементов на противоположные
    for (size_t i = 0; i < elementsToReplace; ++i) 
    {
        newArr[n - 1 - i] = -newArr[n - 1 - i]; // Заменяем элемент на противоположный
    }

    cout << "Новый массив с замененными элементами: ";
    for (size_t i = 0; i < n; ++i)
    {
        cout << newArr[i] << " ";
    }
